...
get_soil_features <- function(plt){
  ...
  bbclip <- st_bbox(centers) %>% as.numeric
  ...
  soil_geometry_to_clip <- mapunit_geom_by_ll_bbox(bbclip)
  soil_geometry_to_clip <- sf::st_as_sf(soil_geometry_to_clip)
  sf::st_crs(soil_geometry_to_clip)<- 4326
  #clip
  soil_features <- sf::st_join(centers, soil_geometry_to_clip, join = st_intersects)
