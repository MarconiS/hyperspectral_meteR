...
setwd("..")
nComp = 10
invisible(base_data <- readr::read_csv("./indir/leaf_traits_and_aop.csv"))
...
local_soil <- c("musym")
# soil_features <- soilDB::SDA_query(q)
...
list_formulas <- paste(paste("mvbind("
                       ...
                       #climate variables
                       , paste(colnames(climate_features)[2:8], collapse = " + ", sep = "") , " + "
...
formula_reflectance_only <- paste(paste("mvbind("
                       , paste(colnames(leaf_traits_data)[-1], collapse = " , "), ") ~ ", sep = "")
                       , paste(colnames(reflectance_data)[-1], collapse = " + ", sep = "") , " + "
                       , "(1 | site | siteID"
                       , ")", collapse = "+")
...
fit <- brm(list_formulas, data = train_dataset, cores =2, chains = 2, iter = 2000,  
           family=brmsfamily("student"), cov_ranef = list(taxonID = as.matrix(taxaPD$mat)))

refl_fit <- brm(formula_reflectance_only, data = train_dataset, cores =2, chains = 2, iter = 2000,  
           family=brmsfamily("student"), prior = prior(horseshoe(), class = "b"))
...
test_dataset = test_dataset[complete.cases(train_dataset),]
```

```{r}
#test model

bR2 <- bayes_R2(refl_fit, new_data= test_dataset)
```